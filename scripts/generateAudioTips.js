/**
 * This script generates audio files (syllables and full words) for flashcard items
 * using the ElevenLabs Text-to-Speech API.
 * 
 * It reads items from `flashcards_base.json`, extracts or generates syllable tips,
 * checks for existing audio files, and calls the ElevenLabs API to generate missing
 * audio files (in multiple samples/variations).
 * 
 * Requires ELEVENLABS_API_KEY and optionally ELEVENLABS_VOICE_ID in the .env file.
 * 
 * Arguments:
 * --test: Runs in test mode (limits generation, useful for quick checks).
 * --limit=<number>: Limits the number of files generated per type (syllable/word).
 * --force: Forces regeneration of audio files even if they already exist.
 */
